...
PATTERN_RULES = [
    ...
    ("no-unsized-array", "warn", re.compile(r"\bfloat(?:64)?\s*\[\s*\]"),
     "Unsized `float[]` compiles (it is a slice) but is an EMPTY view, not a buffer — "
     "use a fixed-size `float[N]` for delay/state buffers."),
